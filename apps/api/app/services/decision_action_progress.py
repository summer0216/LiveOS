...
class DecisionActionProgressService:
    ...
    @staticmethod
    def _latest_verified_from(
        state: DecisionActionState,
    ) -> LatestVerifiedAction | None:
        if (
            state.status != ActionProgressStatus.COMPLETED
            or state.outcome_status is None
            or not state.verification_evidence
        ):
            return None
    ...
    def apply_update(
        ...
        with self._lock:
            ...
            state = decision_action_state_store.save(
                state.model_copy(
                    update={
                        "status": (update.status if update.relevant else state.status),
                        "outcome_status": (
            ...
            if outcome_update.relevant:
                verified_action = self._latest_verified_from(state)
    ...
    def latest_verified_state(
        self,
        conversation_id: str,
    ) -> LatestVerifiedAction | None:
        return latest_verified_action_store.get(conversation_id)

    def delete_conversation(self, conversation_id: str) -> None:
        decision_action_state_store.delete_conversation(conversation_id)
        latest_verified_action_store.delete_conversation(conversation_id)
